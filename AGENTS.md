...
- **A worker may have its own `AGENTS.md`** (with a `CLAUDE.md` symlink, like the root). Worker-specific rules there OVERRIDE these norms — read it before touching that worker. Example: `twitter-bookmarks/` forbids the standard local-exec verify loop because of its token handling.
